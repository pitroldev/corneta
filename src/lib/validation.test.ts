...
describe("sanitizeStreamKey", () => {
  it("chave pura só é trimada", () => {
    expect(sanitizeStreamKey("  live_123  ")).toEqual({ key: "live_123", strippedUrl: false });
  });
  it("recorta a chave de uma URL colada inteira", () => {
    expect(sanitizeStreamKey("rtmp://live.twitch.tv/app/live_123").key).toBe("live_123");
  });
  it("preserva a querystring (bandwidthtest da Twitch)", () => {
    expect(sanitizeStreamKey("rtmp://live.twitch.tv/app/live_9?bwtest=true").key).toBe("live_9?bwtest=true");
  });
  it("só o servidor (== ingestUrl) → sem chave, strippedUrl", () => {
    const r = sanitizeStreamKey("rtmp://live.twitch.tv/app", "rtmp://live.twitch.tv/app");
...
describe("sanitizeApiKey", () => {
  it("extrai a AIza… de qualquer sujeira", () => {
    ...
    expect(sanitizeApiKey(`"${KEY}"`)).toBe(KEY);
    expect(sanitizeApiKey("https://console.cloud.google.com/x?key=" + KEY)).toBe(KEY);
  });
...
describe("sanitizeToken", () => {
  it("tira Bearer, aspas, rótulo e query", () => {
    ...
    expect(sanitizeToken("https://sockets.streamlabs.com/?token=abc")).toBe("abc");
  });
});
