...
const names = (config: AppConfig): Set<string> => new Set(config.targets.map((t) => t.name));

export function addTarget(config: AppConfig, platformId: PlatformId): { config: AppConfig; id: string } {
  const t = makeTarget(platformId);
  t.name = suffixName(t.name, names(config));
  return { config: { ...config, targets: [...config.targets, t] }, id: t.id };
}

export function updateTarget(config: AppConfig, id: string, patch: Partial<Target>): AppConfig {
  return { ...config, targets: config.targets.map((t) => (t.id === id ? { ...t, ...patch } : t)) };
}
...
export function insertTarget(config: AppConfig, target: Target, index: number): AppConfig {
...
export function reorderTargets(config: AppConfig, ordered: Target[]): AppConfig {
  return { ...config, targets: ordered };
}

/** Duplica um destino (sem a chave; nome com sufixo) logo após o original. `newId` vem de fora. */
export function duplicateTarget(config: AppConfig, id: string, newId: string): AppConfig {
  const index = config.targets.findIndex((t) => t.id === id);
...
export function moveTarget(config: AppConfig, id: string, dir: -1 | 1): AppConfig {
...
export function addProfile(config: AppConfig, id: string): AppConfig {
  ...
  return { ...config, profiles: [...config.profiles, prof], activeProfileId: id };
}
...
export function renameProfile(config: AppConfig, id: string, name: string): AppConfig {
  return {
    ...config,
