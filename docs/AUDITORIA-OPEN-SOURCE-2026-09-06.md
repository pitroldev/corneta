...
## Estado após as correções P0

Atualizado em 6 de setembro de 2026. **As correções locais foram implementadas e validadas; nem todos os P0 operacionais estão encerrados.** A publicação de instaladores continua bloqueada. Não houve commit, push, release ou mudança de visibilidade nesta implementação.
...
| OS-20, condicional | Pin acessível; durabilidade pendente           | Binário exato estava acessível e a opção de espelho preserva o hash. Falta escolher/provisionar armazenamento durável; não foi publicado espelho.                                                                                      |
| OS-27              | Parcial; distribuição bloqueada                | Fontes exatas do FFmpeg e receitas BtbN obtidas por SHA, hasheadas e registradas. Faltam fontes das bibliotecas/transitivos e revisão do conjunto. `reviewed: false` mantido.                                                          |
...
**Evidência:** há [`lib/i18n/pt.ts`](../lib/i18n/pt.ts) e [`lib/i18n/en.ts`](../lib/i18n/en.ts) na raiz, além dos dicionários ativos em `src/lib/i18n/` e `web/lib/i18n/`. Os arquivos da raiz não são cópias byte a byte dos atuais. Os `tsconfig` inspecionados cobrem `src` e `web`, não esse diretório da raiz; não encontrei consumidores externos desses dois arquivos na busca de imports realizada.
