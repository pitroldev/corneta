...
## Matriz real ainda necessária

| Medição                          | Procedimento reproduzível                                                                                                                                 | Estado                                                              |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Startup frio/quente              | Versão/SHA do instalador; perfil vazio versus existente; 10 partidas por condição; tempo até interação útil, p50/p95. Separar updater e rede.             | Pendente.                                                           |
| Abrir relatório e marcar momento | Mesmo relatório/volume; medir clique → feedback/primeiro conteúdo e conclusão de análise separadamente; confirmar player sem remontagem.                  | Smoke cobre regressões funcionais; baseline humana/nativa pendente. |
| Chat sob carga                   | Mesmo ritmo/volume e número de canais; medir resposta ao scroll/envio, elementos montados e retenção; não confundir mensagens recebidas com renderizadas. | Gates sintéticos, validação nativa pendente.                        |
| CPU/GPU e memória                | OBS/sinal idênticos, resolução/FPS, destinos, cópia/híbrido/encode; Corneta e cada sidecar separados, incluindo memória GPU.                              | Pendente por hardware/driver.                                       |
| Live longa                       | Pelo menos 2 h no ensaio inicial; capturar série após aquecimento e comparar crescimento sustentado com volume e limites.                                 | Pendente; não inferir leak de um único pico de RSS.                 |
