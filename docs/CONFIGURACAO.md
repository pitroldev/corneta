...
O site usa `/download` sem variável de ambiente ou segredo: resolve o `latest.json` público da release oficial para o instalador Windows `.exe` versionado, validando repositório, plataforma e versão. O redirecionamento válido pode ficar em cache no CDN por cinco minutos; manifesto inválido ou indisponível encaminha para a página oficial `releases/latest`, sem cache. O [atualizador do desktop](ATUALIZACAO-AUTOMATICA.md) mantém seu endpoint e sua verificação de assinatura, independentemente do site.
